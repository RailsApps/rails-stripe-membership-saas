...
You can change the roles and name, email, and password as you wish. See the advice below about using "example.com" email addresses.
...
The application handles "example.com" email addresses as a special case. Users with an "example.com" domain will not be added to Stripe as subscribers; they will only be added to the application database. This makes it possible to run @rake db:seed@ to set up the administrator's account and any sample users for development and testing.

If you test subscribing a new user (providing a fake credit card number such as _4242424242424242_), don't use an "example.com" email address if you want to see the results in the Stripe administrative dashboard.

If you wish to special case a different domain for email addresses (such as your own for an administrator's account), look at the *app/models/user.rb* file. Change the statement in the @update_stripe@ method:

<pre>
return if email.include?('@example.com') and not Rails.env.production?
</pre>

As you can see from the code, the "example.com" email addresses will not be special cased in production. That means @rake db:seed@ will fail with errors if you attempt to run it on Heroku after deployment. See advice below about deploying to Heroku.
...
You'll store the API and public key in an initializer file.
...
Instead, rename the file *config/local_env.example.yml* to *config/local_env.yml*.

Open *config/local_env.yml* and replace "Your_Stripe_API_Key" and "Your_Stripe_Public_Key" with the keys provided by Stripe.

See the article "Rails Environment Variables":http://railsapps.github.com/rails-environment-variables.html for more information. As an alternative to using the *config/local_env.yml* file, the article shows how to set environment variables in the Unix shell.

Here's how to set Heroku environment variables to provide the same data:

<pre>
$ heroku config:add STRIPE_API_KEY=secret STRIPE_PUBLIC_KEY=secret
</pre>

You can't set Heroku environment variables now; you'll do that after you deploy. If you plan to use Heroku for hosting, make a note for yourself to set up the Heroku environment variables after deployment.
...
You'll need to set your Stripe API and public key as Heroku environment variables. See the article "Rails Environment Variables":http://railsapps.github.com/rails-environment-variables.html for more information. Here's how to set Heroku environment variables for your Stripe API and public key:

<pre>
$ heroku config:add STRIPE_API_KEY=secret STRIPE_PUBLIC_KEY=secret
</pre>

Prior to deployment, it's advisable to change your *db/seeds.rb* file. At a minimum, change the "example.com" address for the administrator and remove the "example.com" sample users. Optionally, you can change your own name, email and password or wait and change the values through the application "Edit account" interface later. You can also change list of roles that correspond to the subscription plans you will offer.
...
puts 'SETTING UP DEFAULT USER LOGIN'
user = User.create! :name => 'My Name', :email => 'me@mydomain.com', :password => 'please', :password_confirmation => 'please'
user.add_role :admin
puts 'admin created: ' << user.name
</pre>
...
You'll need to special case your administrator's email address in the *app/models/user.rb* file. Change the statement in the @update_stripe@ method:
...
<pre>
return if email.include?('@mydomain.com')
</pre>

If you don't special case your email address, @rake db:seed@ will fail with errors if you attempt to run it on Heroku after deployment, since you are not supplying a credit card for Stripe for the administrator.
