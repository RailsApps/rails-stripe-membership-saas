...
describe 'StripeToken' do
  ...
  describe 'Direct Token Creation' do
    it "generates and reads a card token for create charge" do
      StripeMock.start
      card_token = StripeMock.generate_card_token(exp_month: 9, exp_year: 2017)
      charge = Stripe::Charge.create(amount: 500, card: card_token, currency: :usd)
      card = charge.card
      expect(card).to match(/^test_tok/)
      StripeMock.stop
    end
  ...
  describe "error handling" do
    it "throws an error if neither card nor customer are provided", :live => true do
      expect { Stripe::Token.create }.to raise_error(Stripe::AuthenticationError, /No API key provided. Set your API key using "Stripe.api_key = <API-KEY>"/)
    end
  end
