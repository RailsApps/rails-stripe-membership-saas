# https://github.com/rebelidealist/stripe-ruby-mock/blob/master/spec/shared_stripe_examples/card_examples.rb
require 'stripe_mock'
...
describe 'Card API' do
  ...
  it 'creates/returns a card when using recipient.cards.create given a card token' do
    ...
    card_token = stripe_helper.generate_card_token(last4: "1123", exp_month: 12, exp_year: 2016)
  ...
  it 'create does not change the customers default card if already set' do
    ...
    card_token = stripe_helper.generate_card_token(last4: "1123", exp_month: 12, exp_year: 2019)
    card = customer.sources.create(source: card_token)
  ...
  it 'create updates the customers default card if not set' do
    customer = Stripe::Customer.create(id: 'test_customer_sub')
    card_token = stripe_helper.generate_card_token(last4: "1123", exp_month: 12, exp_year: 2020)
    card = customer.sources.create(source: card_token)
  ...
  describe "retrieval and deletion with customers" do
    ...
    let!(:card_token) { stripe_helper.generate_card_token(last4: "1123", exp_month: 12, exp_year: 2021) }
    ...
    context "deletion when the user has two cards" do
      ...
      let!(:card_token_2) { stripe_helper.generate_card_token(last4: "1123", exp_month: 12, exp_year: 2022) }
  ...
  context "update card" do
    ...
    let!(:customer) { Stripe::Customer.create(id: 'test_customer_sub') }
    let!(:card_token) { stripe_helper.generate_card_token(last4: "1123", exp_month: 12, exp_year: 2028) }
    let!(:card) { customer.sources.create(source: card_token) }
  ...
  context "retrieve multiple cards" do
    ...
    it "retrieves a list of multiple cards" do
      ...
      card_token = stripe_helper.generate_card_token(last4: "1123", exp_month: 12, exp_year: 2029)
